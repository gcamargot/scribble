...
SELECT
    ...
    ROUND(AVG(s.execution_time_ms)::numeric, 2) FILTER (WHERE s.status = 'accepted' AND s.execution_time_ms IS NOT NULL) AS avg_execution_time_ms,
    ROUND(AVG(s.memory_used_kb)::numeric, 2) FILTER (WHERE s.status = 'accepted' AND s.memory_used_kb IS NOT NULL) AS avg_memory_kb,
