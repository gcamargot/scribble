#!/bin/sh
# Rust code executor for Scribble
# Compiles and executes user Rust code against test cases
...
# Decode base64 code
echo "$CODE" | base64 -d > /tmp/solution.rs 2>/dev/null || print_error "compilation_error" "Failed to decode code"

# Compile with timing
compile_start=$(date +%s%3N)
compile_output=$(rustc -O -o /tmp/solution /tmp/solution.rs 2>&1) || {
