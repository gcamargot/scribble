...
# Compiles and executes user C++ code against test cases
...
# Decode base64 code
echo "$CODE" | base64 -d > /tmp/solution.cpp 2>/dev/null || print_error "compilation_error" "Failed to decode code"

# Create wrapper with common includes
cat > /tmp/user_code.cpp << 'HEADER'
#include <iostream>
#include <vector>
#include <string>
#include <algorithm>
...
cat /tmp/solution.cpp >> /tmp/user_code.cpp

# Compile with timing
compile_start=$(date +%s%3N)
compile_output=$(g++ -O2 -std=c++17 -o /tmp/solution /tmp/user_code.cpp 2>&1) || {
...
# For simplicity, we use Python to handle JSON parsing and test execution
